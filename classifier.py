...
class Classifier(nn.Module):
    def __init__(self, input_dim=3584, num_classes=2):
        super().__init__()
...
def train(model, train_loader, val_loader, epochs, ckpt_path, device=torch.device("cuda:0")):
    ...
    model.to(device)
    patience = 10 # 早停 patience 为 3 个 epoch

    best_val_loss = float('inf') # 初始化为无穷大
    patience_counter = 0         # 连续损失上升的计数器
    best_model_path = os.path.join(ckpt_path, "best_classifier_model.pth")
    # 确保保存路径存在
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter += 1 # 损失没有下降，增加计数器
...
def plot_training_history(history, train_process_fig=None):
...
def plot_all_layers(acc_list, pre_list, rec_list, f1_list, save_file):
    x = list(range(1, len(acc_list) + 1))

    plt.figure(figsize=(10, 6))

    plt.plot(x, acc_list, label='Accuracy')
    plt.plot(x, pre_list, label='Precision')
    plt.plot(x, rec_list, label='Recall')
    plt.plot(x, f1_list, label='F1 Score')

    plt.xlabel('Layer Index')
    plt.ylabel('Metric Value')
    plt.title('Model Metrics Over Layers')
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    run_train = True
    # dataset_file = 'data/hidden_states_PS-P_O-splited-Instruct-Mixed_50000.npy'  
    dataset_path = "/model/fangly/mllm/ljd/Internal-View-of-Metaphor/data/hidden_states/"
    dataset_file_list = os.listdir(dataset_path)
    dataset_file_list = [os.path.join(dataset_path, file) for file in dataset_file_list]
    ckpt_path = 'ckpt/classifier/'
    ckpt_file = "best_classifier_model.pth"
    BATCH_SIZE = 64
    EPOCHS = 20
    ...
    for dataset_file in tqdm(dataset_file_list):
        train_loader, val_loader, test_loader = load_and_split_data(data_file=dataset_file, batch_size=BATCH_SIZE)   
        model = Classifier(input_dim=3584, num_classes=2)

        if not run_train:
            # 2. 加载预训练模型权重
            model.load_state_dict(torch.load(os.path.join(ckpt_path, ckpt_file), map_location=DEVICE))
            evaluate_model(model, test_loader, device=DEVICE)

        else:
            # 3. 开始训练
            base_name = os.path.basename(dataset_file).replace(".npz", "")
            history, model = train(model, train_loader, val_loader, epochs=EPOCHS, ckpt_path=os.path.join(ckpt_path, base_name), device=DEVICE)
            # 4. 可视化损失变化
            train_process_fig = os.path.join(ckpt_path, base_name, "training_history.png")
            plot_training_history(history, train_process_fig)
    ...
    plot_all_layers(acc_list, pre_list, rec_list, f1_list, save_file=os.path.join(ckpt_path, "metrics_over_layers.png"))
